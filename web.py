...
""" Thread-safety notes
通常，RequestHandler和tornado其他地方的方法都是非线程安全的。特别地，诸如write(),finish(),flush()这些方法只能在主线程中调用。
如果使用了多线程，则需要在完成请求之前，先使用IOLoop.add_callback()方法把控制权转移回主线程中。 """
...
class RequestHandler(object):
    """ 继承这个类并定义诸如get(),post()等方法以实现一个handler。 """

    # 如果支持的方法超出了SUPPORTED_METHODS里的范围，则需要在子类中重新定义这个变量。
...
class Application(object):
    """ 请求handler的集合，构成整个web应用。该类的实例是可调用的，且可以直接传递给HTTPServer从而服务该应用。
    该类的构造函数以URLSpec对象或(正则，handler类)tuple的列表为参数，当收到请求时，会用遍历该列表找到的匹配路径的第一个类创建对象。
    每一个tuple可以用一个dict作为第三个参数。该字典会以keyword参数的形式传递给该handler的构造函数。
    该类的add_handlers支持虚拟hosts，它以一个host的正则作为第一个参数。 """
    ...
    def listen(self, port, address="", **kwargs):
        """ 在给定的端口上启动HTTP服务器。这只是简单调用了一下HTTPServer并listen。 """
        # import is here rather than top level because HTTPServer is not importable on appengine
    ...
    def add_handlers(self, host_pattern, host_handlers):
        ...
        Note that host patterns are processed sequentially in the order they were added,
        and only the first matching pattern is used.
        This means that all handlers for a given host must be added in a single add_handlers call.
        """
    ...
    def __call__(self, request): # HTTPServer接收到请求之后的回调函数，request参数是由HTTPServer分析请求后包装出来的
        """ HTTPServer会调用该类的对象以执行一次请求。 """
            ...
            for spec in handlers:
                ...
                if match:
                    ...
                    if spec.regex.groups:
                        ...
                        # Pass matched groups to the handler.  Since match.groups() includes both named and unnamed groups,
                        # we want to use either groups or groupdict but not both.
                        # Note that args are passed as bytes so the handler can decide what encoding to use.

                        if spec.regex.groupindex:
                            kwargs = dict((str(k), unquote(v)) for (k, v) in match.groupdict().iteritems())
                        else:
        ...
        # In debug mode, re-compile templates and reload static files on every request so you don't need to restart to see changes
